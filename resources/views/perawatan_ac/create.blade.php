...
@section('title', 'Input Perawatan AC')

@section('content')
<div class="container py-4">
    <h1 class="page-title mb-4">Input Perawatan AC</h1>
    ...
    <form action="{{ route('perawatan-ac.store') }}" method="POST" enctype="multipart/form-data" class="card p-4 shadow-sm rounded-4">
        @csrf
            ...
            <div class="mb-3 checklist-perawatan">
                ...
                <div class="row">
                    ...
                    @foreach($indoorMaint as $item)
                        <div class="col-md-6">
                            <div class="form-check">
                                <input class="form-check-input" type="checkbox" name="perawatan[]" value="{{ $item }}" id="{{ strtolower(str_replace(' ', '_', $item)) }}">
                                <label class="form-check-label" for="{{ strtolower(str_replace(' ', '_', $item)) }}">{{ $item }}</label>
                            </div>
                        </div>
                    @endforeach
